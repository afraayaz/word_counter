...
let regix =
  /\s+/g; /*regular expression to remove whitespaces from text / mark the start and end,
...
console.log(
  chalk.yellow("No of characters in paragraph: " + charWithoutSpaces.length)
);
let wordWithoutSpaces =
  answer.text.match(
    /\S+/g
  ); /*match to find all sequences of non-whitespace characters (words) in the paragraph and
store the word into an array*/
console.log(
  chalk.yellow("No of words in paragraph: " + wordWithoutSpaces.length)
);
